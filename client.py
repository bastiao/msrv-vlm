...
def upload_image(image_path, prompt, return_format="json", api_url="http://127.0.0.1:5000/upload"):
    """
    ...
    Args:
        ...
        return_format (str): The desired return format (json or json-ld).
        api_url (str): The URL of the API endpoint.
    """
    try:
        with open(image_path, 'rb') as image_file:
            files = {'file': (image_path.split('/')[-1], image_file, 'image/png')}
            data = {'prompt': prompt, 'return_format': return_format}
...
if __name__ == "__main__":
    ...
    parser.add_argument('return_format', type=str, choices=['json', 'json-ld'], default="json", help="The desired return format (json or json-ld).")
    parser.add_argument('api_url', type=str, nargs='?', default="http://127.0.0.1:5000/upload", help="The URL of the API endpoint.")
    
    args = parser.parse_args()
    
    upload_image(args.image_path, args.prompt, args.return_format, args.api_url)